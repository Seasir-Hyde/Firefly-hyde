...
function createCard(m: { author?: string; avatar?: string; published: string; content?: string; images?: unknown; tags?: string[]; location?: string; pinned?: boolean }): HTMLElement {
...
function createPinnedPreview(m: { content?: string; images?: unknown; published?: string }): HTMLElement {
...
function insertExternalMoments(extMoments: ExternalMoment[]): void {
  ...
  extMoments.sort((a, b) => new Date(b.published).getTime() - new Date(a.published).getTime());
...
function insertPinnedExternal(extMoments: ExternalMoment[]): void {
  ...
  pinned.sort((a, b) => new Date(b.published).getTime() - new Date(a.published).getTime());
  pinned.forEach(m => {
  ...
  if (countEl) {
    const existing = parseInt(countEl.textContent || "0") || 0;
...
function fetchMoments(): void {
  ...
  fetch("https://api.github.com/gists/" + gistId, { headers })
    ...
    .then(gist => {
      ...
      (window as unknown as Record<string, MomentsCache>)[cacheKey] = { data: moments, time: Date.now() };
